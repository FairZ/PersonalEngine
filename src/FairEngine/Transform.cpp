...
{
	//recursively generate a rotation matrix
	glm::mat4x4 retVal(1);
	if (!m_parent.expired())
	{
...
void Transform::SetPosition(glm::vec3 _position)
{
	if(!GetParent().expired())
	{
		//multiply by the inverse of the parent's transformation matrix to convert it to local space
...
void Transform::SetRotation(glm::vec3 _rotationInRadians)
{
	if(!GetParent().expired())
	{
		//remove rotation of parents to rotate by the correct amount
...
void Transform::SetScale(glm::vec3 _scale)
{
	if(!GetParent().expired())
	{
		//remove scale of parents to scale by the correct amount
...
void Transform::DetachChildren()
{
	for (unsigned int i = 0; i < m_children.size(); i++)
	{
		...
		if(!child.expired())
		{
			//set the childs transformation to their transformation in world space
...
void Transform::DetachFromParent()
{
	if(!m_parent.expired())
	{
		//set transformation to transformation in world space
